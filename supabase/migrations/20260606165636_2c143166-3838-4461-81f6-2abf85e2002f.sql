...
BEGIN
  ...
  RETURN QUERY
  WITH base AS (
    SELECT p.id AS uid, p.display_name AS dname, p.photo_url AS purl
  ...
  ranked AS (
    SELECT
      rank() OVER (ORDER BY a.score DESC, a.uid) AS rnk,
      a.*
    FROM agg a
  )
  SELECT
    ...
    coalesce(ranked.dname, 'Anonyme') AS display_name,
    ranked.purl AS photo_url,
    ranked.score,
    ranked.tasks_done,
    ranked.habits_done,
