...
const formSchema = z.object({
  ...
  password: z.string().min(8, { message: "Le mot de passe doit contenir au moins 8 caractères." }).max(100, { message: "Le mot de passe est trop long." }).regex(/[A-Z]/, { message: "Le mot de passe doit contenir au moins une majuscule." }).regex(/[a-z]/, { message: "Le mot de passe doit contenir au moins une minuscule." }).regex(/[0-9]/, { message: "Le mot de passe doit contenir au moins un chiffre." }),
  displayName: z.string().min(2, { message: "Le nom d'utilisateur doit contenir au moins 2 caractères." }).max(50, { message: "Le nom d'utilisateur est trop long." }).regex(/^[a-zA-Z0-9\s\-_]+$/, { message: "Le nom d'utilisateur ne peut contenir que des lettres, chiffres, espaces, tirets et underscores." }),
});
...
export default function Register() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Erreur lors de l'inscription", description: error?.message || "Une erreur s'est produite. Veuillez réessayer." });
    } finally {
      setIsLoading(false);
    }
  }
